...
const HOST_CITIES_COORDS = {
  ...
  Dallas: { x: 48, y: 62, lat: 32.7767, lng: -96.797 },
  Houston: { x: 50, y: 70, lat: 29.7604, lng: -95.3698 },
  Atlanta: { x: 68, y: 58, lat: 33.749, lng: -84.388 },
  Miami: { x: 78, y: 78, lat: 25.7617, lng: -80.1918 },
  "New York": { x: 84, y: 31, lat: 40.7128, lng: -74.006 },
  Philadelphia: { x: 82, y: 35, lat: 39.9526, lng: -75.1652 },
  Boston: { x: 86, y: 25, lat: 42.3601, lng: -71.0589 },
  "New York/New Jersey": { x: 84, y: 31, lat: 40.7128, lng: -74.006 },
...
export default function MapView({
  ...
  useEffect(() => {
    if (googleMapsLoaded && mapRef.current && !mapInstance) {
      const centerCoord =
        HOST_CITIES_COORDS[centerCity] || HOST_CITIES_COORDS["Miami"];
  ...
  const getSvgCoordinates = (marker) => {
    if (marker.city && HOST_CITIES_COORDS[marker.city]) {
      ...
      const seed = marker.id
        ? marker.id.charCodeAt(0) + (marker.id.charCodeAt(1) || 0)
        : 0;
      const jitterX = marker.type !== "city" ? ((seed % 7) - 3) * 0.8 : 0;
      const jitterY =
        marker.type !== "city" ? (((seed >> 1) % 7) - 3) * 0.8 : 0;
  ...
  return (
    <MapContext.Provider value={contextValue}>
      <div
        id="scout-map-wrapper"
        style={{ position: "relative", width: "100%", marginBottom: "1.5rem" }}
      >
          ...
          >
            ...
            >
              ...
              {activePaths.map((path) => {
                ...
                const coordinates = path.stops
                  .map((stopName) => HOST_CITIES_COORDS[stopName])
                  .filter(Boolean);
            ...
            {activeMarkers.map((marker) => {
              ...
              const isSelected =
                selectedElement && selectedElement.id === marker.id;
              ...
              return (
                ...
                >
                  ...
                  >
                    {content}
                    {marker.type === "city" && marker.team && (
                      <div style={{ display: "none", fontSize: "13px" }}>
                        ⚽
                      </div>
                    )}
                  </div>
              ...
              >
                ...
                >
                  {selectedElement.type === "hotel" && (
                    <>
                      <div>
                        <strong>Rating:</strong> ⭐ {selectedElement.rating}
                      </div>
                      <div>
                        <strong>Price:</strong>{" "}
                        {"$".repeat(selectedElement.price_level)}
                      </div>
                      <div>
                        <strong>Distance:</strong> {selectedElement.distance}
                      </div>
                  ...
                  {selectedElement.type === "city" && (
                    <>
                      <div>
                        <strong>City:</strong> {selectedElement.city}
                      </div>
                      <div>
                        <strong>Stadium:</strong>{" "}
                        {selectedElement.stadium || "Host Stadium"}
                      </div>
                      <div>
                        <strong>Match:</strong>{" "}
                        {selectedElement.match || "Group Stage"}
                      </div>
                      <div>
                        <strong>Date:</strong> {selectedElement.date}
                      </div>
                      {selectedElement.team && (
                        <div>
                          <strong>Fan profile:</strong> {selectedElement.team}{" "}
                          route marker
                        </div>
                      )}
                      <div
                      ...
                      >
                        Welcome to {selectedElement.city}! Follow{" "}
                        {selectedElement.team || "the team"}'s journey to{" "}
                        {selectedElement.stadium}.
                      </div>
                    </>
                  )}
